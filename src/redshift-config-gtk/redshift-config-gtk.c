...
static int method_try_start(const gamma_method_t *method, gamma_state_t **state,
                            config_ini_state_t *config, gchar **args) {
  int r;
...
static gboolean validate_dawn_dusk_settings() {
  ...
  if (dawn_start > dawn_end) {
    GtkWidget *d = gtk_message_dialog_new(
        gtk_application_get_active_window(app), GTK_DIALOG_DESTROY_WITH_PARENT,
    ...
    gtk_window_set_title(GTK_WINDOW(d),
                         _("Invalid dawn/dusk time configuration"));
...
static gboolean validate_crtcs_setting() {
  gboolean result = TRUE;
  GRegex *regex_valid =
      g_regex_new("^[0-9]+(,[0-9]+)*$", G_REGEX_DOLLAR_ENDONLY, 0, NULL);
  ...
  if (!g_regex_match_all(regex_valid, crtcs, 0, NULL)) {
    ...
    gtk_notebook_set_current_page(GTK_NOTEBOOK(gtk_bin_get_child(GTK_BIN(win))),
                                  2);
    GtkWidget *d = gtk_message_dialog_new(
        win, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_CLOSE,
        _("crtc list (%s) must be comma-separated list of numbers."), crtcs);
    gtk_window_set_title(GTK_WINDOW(d), _("Invalid crtc configuration"));
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(d);
    result = FALSE;
  }
...
static void disable_revert_menuitem() {
  g_action_map_remove_action(G_ACTION_MAP(app), "revert");
}

static void write_config_file(const gchar *outfile) {
  ...
  GFileOutputStream *fos_tmp =
      g_file_replace(gfile_tmp, NULL, TRUE, G_FILE_CREATE_NONE, NULL, &err);
  if (fos_tmp == NULL) {
    GtkWidget *d = gtk_message_dialog_new(
        gtk_application_get_active_window(app), GTK_DIALOG_DESTROY_WITH_PARENT,
        GTK_MESSAGE_ERROR, GTK_BUTTONS_CLOSE,
  ...
  /* Settings */
  const gchar *selected_method_id =
      gtk_combo_box_get_active_id(GTK_COMBO_BOX(cfg.combo_method));
  const gchar *selected_provider_id =
      gtk_combo_box_get_active_id(GTK_COMBO_BOX(cfg.combo_provider));
  gchar *temperature_day_config = g_strdup_printf(
      "temp-day=%d\n", (gint)gtk_adjustment_get_value(cfg.adj_temperature[0]));
  gchar *temperature_night_config =
      g_strdup_printf("temp-night=%d\n",
                      (gint)gtk_adjustment_get_value(cfg.adj_temperature[1]));
  gchar *brightness_day_config = g_strdup_printf(
      "brightness-day=%.3f\n", gtk_adjustment_get_value(cfg.adj_brightness[0]));
  gchar *brightness_night_config =
      g_strdup_printf("brightness-night=%.3f\n",
                      gtk_adjustment_get_value(cfg.adj_brightness[1]));
  gchar *gamma_day_config =
      g_strdup_printf("gamma-day=%.3f:%.3f:%.3f\n",
  ...
  gchar *gamma_night_config =
      g_strdup_printf("gamma-night=%.3f:%.3f:%.3f\n",
  ...
  if (cfg.opts.scheme.use_time) {
    gchar *dawn_start =
        g_strdup_printf("%02d:%02d", cfg.opts.scheme.dawn.start / 3600,
                        cfg.opts.scheme.dawn.start % 3600 / 60);
    gchar *dawn_end = NULL;
    if (cfg.opts.scheme.dawn.start != cfg.opts.scheme.dawn.end) {
      dawn_end = g_strdup_printf("-%02d:%02d", cfg.opts.scheme.dawn.end / 3600,
    ...
    g_free(dawn_start);
    gchar *dusk_start =
        g_strdup_printf("%02d:%02d", cfg.opts.scheme.dusk.start / 3600,
                        cfg.opts.scheme.dusk.start % 3600 / 60);
    gchar *dusk_end = NULL;
    if (cfg.opts.scheme.dusk.start != cfg.opts.scheme.dusk.end) {
      dusk_end = g_strdup_printf("-%02d:%02d", cfg.opts.scheme.dusk.end / 3600,
  ...
  gchar *elevation_high_config =
      g_strdup_printf("elevation-high=%.2f\n", cfg.opts.scheme.high);
  gchar *elevation_low_config =
      g_strdup_printf("elevation-low=%.2f\n", cfg.opts.scheme.low);
  gchar *method_config =
      (strcmp(selected_method_id, "auto") == 0)
          ? NULL
  ...
  if (strcmp(selected_provider_id, "auto") != 0) {
    provider_config =
        g_strdup_printf("location-provider=%s\n", selected_provider_id);
  ...
  if (dawn_config)
    g_string_append(contents, dawn_config);
  if (dusk_config)
    g_string_append(contents, dusk_config);
  g_string_append(contents, elevation_high_config);
  g_string_append(contents, elevation_low_config);
  if (method_config)
    g_string_append(contents, method_config);
  if (provider_config)
    g_string_append(contents, provider_config);
  ...
  GDataOutputStream *dos_tmp =
      g_data_output_stream_new(G_OUTPUT_STREAM(fos_tmp));
...
static void revert_cb() {
  cfg.opts.scheme.day = cfg.color_settings_revert[0];
  cfg.opts.scheme.night = cfg.color_settings_revert[1];

  gtk_adjustment_set_value(cfg.adj_temperature[0],
                           cfg.opts.scheme.day.temperature);
  gtk_adjustment_set_value(cfg.adj_temperature[1],
                           cfg.opts.scheme.night.temperature);
  gtk_adjustment_set_value(cfg.adj_brightness[0],
                           cfg.opts.scheme.day.brightness);
  gtk_adjustment_set_value(cfg.adj_brightness[1],
                           cfg.opts.scheme.night.brightness);
  for (int i = 0; i < 3; i++) {
    gtk_adjustment_set_value(cfg.adj_gamma[0][i], cfg.opts.scheme.day.gamma[i]);
    gtk_adjustment_set_value(cfg.adj_gamma[1][i],
                             cfg.opts.scheme.night.gamma[i]);
  }

  disable_revert_menuitem();
}

static void quit_cb() { g_application_quit(G_APPLICATION(app)); }

static void enable_revert_menuitem() {
  const GActionEntry act_entries[] = {{"revert", revert_cb}};
  g_action_map_add_action_entries(G_ACTION_MAP(app), act_entries,
                                  G_N_ELEMENTS(act_entries), NULL);
}

static void check_fade_toggled_cb(GtkToggleButton *togglebutton) {
  cfg.opts.use_fade = !gtk_toggle_button_get_active(togglebutton);
}

static void check_set_dawn_dusk_toggled_cb(GtkToggleButton *togglebutton) {
  cfg.opts.scheme.use_time = gtk_toggle_button_get_active(togglebutton);
}

static void temperature_value_changed_cb(GtkAdjustment *adjustment) {
...
static void brightness_value_changed_cb(GtkAdjustment *adjustment) {
  color_setting_t *color_setting =
      cfg.active_page ? &cfg.opts.scheme.night : &cfg.opts.scheme.day;
...
static void gamma_value_changed_cb(GtkAdjustment *adjustment,
                                   gpointer user_data) {
...
static void block_adjustment_handlers(guint page) {
  g_return_if_fail(page == 0 || page == 1);

  g_signal_handler_block(cfg.adj_temperature[page], cfg.handler_ids[page * 5]);
  g_signal_handler_block(cfg.adj_brightness[page],
                         cfg.handler_ids[page * 5 + 1]);
  for (int i = 0; i < 3; i++) {
    g_signal_handler_block(cfg.adj_gamma[page][i],
                           cfg.handler_ids[page * 5 + 2 + i]);
  }
}

static void unblock_adjustment_handlers(guint page) {
  g_return_if_fail(page == 0 || page == 1);

  g_signal_handler_unblock(cfg.adj_temperature[page],
                           cfg.handler_ids[page * 5]);
  g_signal_handler_unblock(cfg.adj_brightness[page],
                           cfg.handler_ids[page * 5 + 1]);
  for (int i = 0; i < 3; i++) {
    g_signal_handler_unblock(cfg.adj_gamma[page][i],
                             cfg.handler_ids[page * 5 + 2 + i]);
  }
}

static void connect_adjustment_signals() {
  for (int i = 0; i < 2; i++) {
    cfg.handler_ids[i * 5] =
        g_signal_connect(cfg.adj_temperature[i], "value-changed",
                         G_CALLBACK(temperature_value_changed_cb), NULL);
    cfg.handler_ids[i * 5 + 1] =
        g_signal_connect(cfg.adj_brightness[i], "value-changed",
                         G_CALLBACK(brightness_value_changed_cb), NULL);
    for (int j = 0; j < 3; j++) {
...
static void init_adjustments() {
  cfg.adj_temperature[0] =
      gtk_adjustment_new(cfg.opts.scheme.day.temperature, MIN_TEMPERATURE,
                         MAX_TEMPERATURE, 1, 10, 0);
  cfg.adj_temperature[1] =
      gtk_adjustment_new(cfg.opts.scheme.night.temperature, MIN_TEMPERATURE,
                         MAX_TEMPERATURE, 1, 10, 0);
  cfg.adj_brightness[0] =
      gtk_adjustment_new(cfg.opts.scheme.day.brightness, MIN_BRIGHTNESS,
                         MAX_BRIGHTNESS, 0.001, 0.01, 0);
  cfg.adj_brightness[1] =
      gtk_adjustment_new(cfg.opts.scheme.night.brightness, MIN_BRIGHTNESS,
                         MAX_BRIGHTNESS, 0.001, 0.01, 0);
  for (int i = 0; i < 3; i++) {
    cfg.adj_gamma[0][i] = gtk_adjustment_new(
        cfg.opts.scheme.day.gamma[i], MIN_GAMMA, MAX_GAMMA, 0.001, 0.01, 0);
    cfg.adj_gamma[1][i] = gtk_adjustment_new(
        cfg.opts.scheme.night.gamma[i], MIN_GAMMA, MAX_GAMMA, 0.001, 0.01, 0);
  }
  cfg.adj_latitude = gtk_adjustment_new(0, -90, 90, 0.01, 0.01, 0);
  cfg.adj_longitude = gtk_adjustment_new(0, -180, 180, 0.01, 0.01, 0);
  cfg.adj_dawn_start_h =
      gtk_adjustment_new(cfg.opts.scheme.dawn.start / 3600, 0, 23, 1, 1, 0);
  cfg.adj_dawn_start_m = gtk_adjustment_new(
      cfg.opts.scheme.dawn.start % 3600 / 60, 0, 59, 1, 1, 0);
  cfg.adj_dawn_end_h =
      gtk_adjustment_new(cfg.opts.scheme.dawn.end / 3600, 0, 23, 1, 1, 0);
  cfg.adj_dawn_end_m =
      gtk_adjustment_new(cfg.opts.scheme.dawn.end % 3600 / 60, 0, 59, 1, 1, 0);
  cfg.adj_dusk_start_h =
      gtk_adjustment_new(cfg.opts.scheme.dusk.start / 3600, 0, 23, 1, 1, 0);
  cfg.adj_dusk_start_m = gtk_adjustment_new(
      cfg.opts.scheme.dusk.start % 3600 / 60, 0, 59, 1, 1, 0);
  cfg.adj_dusk_end_h =
      gtk_adjustment_new(cfg.opts.scheme.dusk.end / 3600, 0, 23, 1, 1, 0);
  cfg.adj_dusk_end_m =
      gtk_adjustment_new(cfg.opts.scheme.dusk.end % 3600 / 60, 0, 59, 1, 1, 0);
  cfg.adj_elevation_high =
      gtk_adjustment_new(cfg.opts.scheme.high, -90, 90, 0.01, 0.01, 0);
  cfg.adj_elevation_low =
      gtk_adjustment_new(cfg.opts.scheme.low, -90, 90, 0.01, 0.01, 0);
...
static void startup_cb() { options_init(&cfg.opts); }
...
static gboolean draw_cb(GtkWidget *widget, cairo_t *cr, gpointer user_data) {
  ...
  colorramp_fill_float(ramp_red, ramp_green, ramp_blue, ramp_size,
                       color_setting);
...
static void switch_page_cb(GtkNotebook *notebook, GtkWidget *page,
  ...
  cfg.opts.method->set_temperature(
      cfg.method_state,
...
static void create_window() {
  ...
  const gchar *period_names[] = {N_("Da_ytime"), N_("_Night")};
  ...
  for (int i = 0; i < 2; i++) {
    ...
    g_object_set_property(G_OBJECT(drawingarea_gammaramp[i]), "margin",
                          &margin);
    ...
    scale_brightness[i] =
        gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, cfg.adj_brightness[i]);
    spin_temperature[i] = gtk_spin_button_new(cfg.adj_temperature[i], 0, 0);
    scale_temperature[i] =
        gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, cfg.adj_temperature[i]);
    ...
    gtk_label_set_mnemonic_widget(GTK_LABEL(label_temperature[i]),
                                  spin_temperature[i]);
    gtk_label_set_mnemonic_widget(GTK_LABEL(label_brightness[i]),
                                  spin_brightness[i]);
    for (int j = 0; j < 3; j++) {
      spin_gamma[i][j] = gtk_spin_button_new(cfg.adj_gamma[i][j], 0, 3);
      scale_gamma[i][j] =
          gtk_scale_new(GTK_ORIENTATION_HORIZONTAL, cfg.adj_gamma[i][j]);
      ...
      gtk_label_set_mnemonic_widget(GTK_LABEL(label_gamma[i][j]),
                                    spin_gamma[i][j]);
    }
    grid_notebook[i] = gtk_grid_new();
    gtk_grid_set_column_spacing(GTK_GRID(grid_notebook[i]), 4);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), drawingarea_gammaramp[i], 0, 0,
                    1, 5);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), label_temperature[i], 1, 0, 1,
                    1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), scale_temperature[i], 2, 0, 1,
                    1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), spin_temperature[i], 3, 0, 1,
                    1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), label_brightness[i], 1, 1, 1,
                    1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), scale_brightness[i], 2, 1, 1,
                    1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), spin_brightness[i], 3, 1, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), label_gamma[i][0], 1, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), scale_gamma[i][0], 2, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), spin_gamma[i][0], 3, 2, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), label_gamma[i][1], 1, 3, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), scale_gamma[i][1], 2, 3, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), spin_gamma[i][1], 3, 3, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), label_gamma[i][2], 1, 4, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), scale_gamma[i][2], 2, 4, 1, 1);
    gtk_grid_attach(GTK_GRID(grid_notebook[i]), spin_gamma[i][2], 3, 4, 1, 1);
    gtk_notebook_append_page(GTK_NOTEBOOK(notebook_general), grid_notebook[i],
                             gtk_label_new_with_mnemonic(_(period_names[i])));
  }
  g_signal_connect(drawingarea_gammaramp[0], "draw", G_CALLBACK(draw_cb),
                   &cfg.opts.scheme.day);
  g_signal_connect(drawingarea_gammaramp[1], "draw", G_CALLBACK(draw_cb),
                   &cfg.opts.scheme.night);

  /* Adjustment method */
  GtkWidget *label_method =
      gtk_label_new_with_mnemonic(_("Adjustment met_hod:"));
  gtk_widget_set_halign(label_method, GTK_ALIGN_START);
  cfg.combo_method = gtk_combo_box_text_new();
  gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(cfg.combo_method), "auto",
                            _("Auto"));
  for (int i = 0; cfg.gamma_methods[i].name != NULL; i++) {
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(cfg.combo_method),
                              cfg.gamma_methods[i].name,
                              cfg.gamma_methods[i].name);
  }
  gtk_label_set_mnemonic_widget(GTK_LABEL(label_method), cfg.combo_method);
  gtk_combo_box_set_active_id(GTK_COMBO_BOX(cfg.combo_method),
                              cfg.opts.method->name);

  /* Location provider */
  GtkWidget *label_provider =
      gtk_label_new_with_mnemonic(_("Location _provider:"));
  gtk_widget_set_halign(label_provider, GTK_ALIGN_START);
  cfg.combo_provider = gtk_combo_box_text_new();
  gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(cfg.combo_provider), "auto",
                            _("Auto"));
  for (int i = 0; cfg.location_providers[i].name != NULL; i++) {
    gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(cfg.combo_provider),
                              cfg.location_providers[i].name,
                              cfg.location_providers[i].name);
  }
  ...
  gtk_combo_box_set_active_id(GTK_COMBO_BOX(cfg.combo_provider),
                              cfg.opts.provider ? cfg.opts.provider->name
                                                : "auto");

  /* Fading */
  GtkWidget *check_disable_fading =
      gtk_check_button_new_with_mnemonic(_("_Disable fading"));
  if (cfg.opts.use_fade == 0) {
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check_disable_fading), TRUE);
  }
  ...
  gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(check_set_dawn_dusk),
                               cfg.opts.scheme.use_time);
  GtkWidget *label_dawn = gtk_label_new_with_mnemonic(_("Da_wn:"));
  GtkWidget *label_dusk = gtk_label_new_with_mnemonic(_("Dus_k:"));
  GtkWidget *spin_dawn_start_h =
      gtk_spin_button_new(cfg.adj_dawn_start_h, 0, 0);
  GtkWidget *spin_dawn_start_m =
      gtk_spin_button_new(cfg.adj_dawn_start_m, 0, 0);
  ...
  GtkWidget *spin_dusk_start_h =
      gtk_spin_button_new(cfg.adj_dusk_start_h, 0, 0);
  GtkWidget *spin_dusk_start_m =
      gtk_spin_button_new(cfg.adj_dusk_start_m, 0, 0);
  ...
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new(":"), 2, 1, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), spin_dawn_start_m, 3, 1, 1, 1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new("-"), 4, 1, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), spin_dawn_end_h, 5, 1, 1, 1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new(":"), 6, 1, 1,
                  1);
  ...
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new(":"), 2, 2, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), spin_dusk_start_m, 3, 2, 1, 1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new("-"), 4, 2, 1,
                  1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), spin_dusk_end_h, 5, 2, 1, 1);
  gtk_grid_attach(GTK_GRID(grid_dawn_dusk_spin), gtk_label_new(":"), 6, 2, 1,
                  1);
  ...
  g_signal_connect(check_set_dawn_dusk, "toggled",
                   G_CALLBACK(check_set_dawn_dusk_toggled_cb), NULL);
  ...
  GtkWidget *spin_elevation_high =
      gtk_spin_button_new(cfg.adj_elevation_high, 0, 2);
  GtkWidget *spin_elevation_low =
      gtk_spin_button_new(cfg.adj_elevation_low, 0, 2);
  ...
  GtkWidget *frame_location =
      gtk_frame_new(_("Location for \"manual\" location provider"));
  ...
  GtkWidget *frame_method_options =
      gtk_frame_new(_("Adjustment method specific options"));
  /* TRANSLATORS: "crtc" is an option name. "randr" is a method name. */
  GtkWidget *label_crtcs =
      gtk_label_new_with_mnemonic(_("crtc (comma-se_parated list) [randr]:"));
  ...
  GtkWidget *label_screen =
      gtk_label_new_with_mnemonic(_("_screen [randr, vidmode]:"));
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook_pages), grid_general,
                           gtk_label_new_with_mnemonic(_("G_eneral")));
  gtk_grid_attach(GTK_GRID(grid_time), frame_dawn_dusk, 0, 4, 2, 1);
  gtk_grid_attach(GTK_GRID(grid_time), frame_elevation, 0, 5, 2, 1);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook_pages), grid_time,
                           gtk_label_new_with_mnemonic(_("Ti_me")));
  ...
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook_pages), grid_advanced,
...
#endif
  gchar *title = g_strdup_printf("%s - %s", cfg.opts.config_filepath,
                                 _("Configure Redshift"));
  ...
  g_signal_connect(check_disable_fading, "toggled",
                   G_CALLBACK(check_fade_toggled_cb), NULL);
  g_signal_connect(notebook_general, "switch-page", G_CALLBACK(switch_page_cb),
                   NULL);
...
static void shutdown_cb(GApplication *application, gpointer user_data) {
...
static gint command_line_cb(GtkApplication *application,
  ...
  if (g_application_command_line_get_is_remote(cmdline)) {
    ...
    GtkWidget *d = gtk_message_dialog_new(
        win, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR,
        GTK_BUTTONS_CLOSE, _("This program is already running."));
    gtk_window_set_title(GTK_WINDOW(d), _("Error"));
    gtk_dialog_run(GTK_DIALOG(d));
    gtk_widget_destroy(d);
  ...
  GVariant *gv_configfile =
      g_variant_dict_lookup_value(options, "configfile", G_VARIANT_TYPE_STRING);
  if (gv_configfile) {
    cfg.opts.config_filepath =
        g_strdup(g_variant_get_string(gv_configfile, NULL));
  }
  ...
  options_parse_config_file(&cfg.opts, &config_state, cfg.gamma_methods,
                            cfg.location_providers);
  ...
  GVariant *gv_method =
      g_variant_dict_lookup_value(options, "method", G_VARIANT_TYPE_STRING);
  if (gv_method) {
    method_args = g_strsplit(g_variant_get_string(gv_method, NULL), ":", -1);
    for (int i = 0; cfg.gamma_methods[i].name != NULL; i++) {
  ...
  if (cfg.opts.method != NULL) {
    ...
    r = method_try_start(cfg.opts.method, &cfg.method_state, &config_state,
                         method_args);
    if (r < 0)
      goto cleanup;
  } else {
    ...
    for (int i = 0; cfg.gamma_methods[i].name != NULL; i++) {
      ...
      r = method_try_start(m, &cfg.method_state, &config_state, NULL);
      if (r < 0) {
  ...
  config_ini_section_t *sect = config_ini_get_section(&config_state, "manual");
  if (sect != NULL) {
  ...
  if (sect != NULL) {
    ...
    while (setting != NULL) {
      if (strcasecmp(setting->name, "crtc") == 0) {
        gtk_entry_set_text(GTK_ENTRY(cfg.entry_crtcs), setting->value);
  ...
  /* Adjust temperature */
  r = cfg.opts.method->set_temperature(cfg.method_state, &cfg.opts.scheme.day,
                                       1);
  if (r < 0) {
...
int main(int argc, char *argv[]) {
  ...
  /* Application */
  app = gtk_application_new("dk.jonls.redshift-config-gtk",
  ...
  /* Actions */
  const GActionEntry act_entries[] = {{"save", save_cb}, {"quit", quit_cb}};
  g_action_map_add_action_entries(G_ACTION_MAP(app), act_entries,
                                  G_N_ELEMENTS(act_entries), NULL);
  ...
  const GOptionEntry opt_entries[] = {
      ...
      {"method", 'm', 0, G_OPTION_ARG_STRING, NULL, help_method, "METHOD"},
      {NULL}};
